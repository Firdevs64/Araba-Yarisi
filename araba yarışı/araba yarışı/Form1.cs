...
namespace araba_yarışı
{
    public partial class Form1 : Form
    {
            ...
            private void AracYerine()
        {
            if(SeritSayisi == 1)
            {
                RedCar.Location = new Point(300,473);
            }
            else if (SeritSayisi == 0)
            {
                RedCar.Location = new Point(90, 473);
            }
            else if (SeritSayisi == 2)
            {
                RedCar.Location = new Point(500, 473);
            }
        }
        ...
        {
            int MuzikDeger = R.Next(1, 4);  //1'den 4'e kadar 4 dahil değil.
        ...
        private void timerRandomCar_Tick(object sender, EventArgs e)
        {
            ...
            for (int i = 0; i < rndCar.Length; i++)
            {
                if (!rndCar[i].FakeHaveCar && rndCar[i].vakit)
                {
                    ...
                    rndCar[i].FakeCar.Top = -rndCar[i].FakeCar.Height;  //Gelen araçtan sonra art arda hemen gelmemesi için yapıyoruz.
                ...
                else
                {
                    if (rndCar[i].vakit)
                    {
                        ...
                        if (rndCar[i].FakeCar.Top >= this.Height - 20)  //amaç : araç düştükten bir süre sonra sonsuza kadar aşağıya gitmeyecek.
                        {
                            rndCar[i].FakeCar.Dispose(); //döngüdeki aracı çıkar anlamında.
        ...
        private void timerSerit_Tick(object sender, EventArgs e)
        {
            ...
            if(SeritHareket == false)
            {
                for(int i = 1; i < 7; i++)
                {
                    this.Controls.Find("labelSolSerit" + i.ToString(), true)[0].Top -= 25; //yukarı doğru hareket sağlar.
                    this.Controls.Find("labelSagSerit" + i.ToString(), true)[0].Top -= 25;
                    //sonsuz döngü yapmak istiyoruz.
                    SeritHareket = true;
                }
            }
            else
            {
                for (int i = 1; i < 7; i++)
                {
                    this.Controls.Find("labelSolSerit" + i.ToString(), true)[0].Top += 25;  //aşağı doğru hareket sağlar.
                    this.Controls.Find("labelSagSerit" + i.ToString(), true)[0].Top += 25;
                    SeritHareket = false;
                }
            }
